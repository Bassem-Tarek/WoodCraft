...
class EmaarRule:
    ...
    def build_plan(self, fr: Frame, p: dict, suffix: str = None) -> dict:
        ...
        ``suffix``, when given, is appended to every parameter name (``wc_lb_N_
        <suffix>`` etc.) so this invocation gets its OWN parameters instead of
        sharing (and silently overwriting) another Line Boring instance's — two
        instances on the same panel, or on different panels, would otherwise both
        drive the same global ``wc_lb_N``/``wc_lb_dia``/etc. and each new run would
        retroactively change every earlier instance's geometry, which is what turns
        a previously-good pattern invalid (red) in the timeline.
        """
        def pn(word: str) -> str:
            return f'{PFX}{word}_{suffix}' if suffix else f'{PFX}{word}'
        ...
        params = [
            (pn('N'),     str(n),                      '',   'Line boring: shelves per column (sets)'),
            (pn('pitch'), f'{_mm(p["pitch"]):.4f} mm', 'mm', 'Line boring: 3-hole set pitch'),
            (pn('dia'),   f'{_mm(p["dia"]):.4f} mm',   'mm', 'Line boring: hole diameter'),
            (pn('depth'), f'{_mm(p["depth"]):.4f} mm', 'mm', 'Line boring: blind hole depth'),
            (pn('front'), f'{_mm(p["front"]):.4f} mm', 'mm', 'Line boring: front-edge setback'),
            (pn('back'),  f'{_mm(p["back"]):.4f} mm',  'mm', 'Line boring: back-panel setback'),
            (pn('shelf'), f'{_mm(shelf):.4f} mm',       'mm', 'Line boring: shelf thickness (taken out of the gaps)'),
            (pn('rise'),  f'{_mm(rise):.4f} mm',        'mm', 'Line boring: shelf underside above the hole centre (pin rise)'),
        ]
        ...
        return {
            ...
            'params': params,
            'seed': seed,
            'back_expr': pn('back'),     # offset from the intersected back-panel line
            'front_expr': pn('front'),   # offset from the projected front edge
            'hole': (pn('dia'), pn('depth')),
            'qty_expr': pn('N'),
            'n_expr': pn('N'),
            'shelf_expr': pn('shelf'),
            'rise_expr': pn('rise'),
